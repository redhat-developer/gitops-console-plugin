...
import {
  AnalysisRunStatusFailureIcon,
  AnalysisRunStatusPendingIcon,
  AnalysisRunStatusRunningIcon,
  AnalysisRunStatusSuccessfulIcon,
  AnalysisRunStatusUnknownIcon,
  MeasurementFailedIcon,
  MeasurementSuccessfulIcon,
...
import { AnalysisRunKind, Measurement, Metric, MetricResult } from '../../model/AnalysisRunModel';
import { AnalysisRunInfo, ReplicaSetInfo } from '../../revisions/ReplicaSetInfo';
import { AnalysisRunStatus } from '../../utils/rollout-utils';
...
export const AnalysisRunStatusFragment: React.FC<AnalysisRunStatusProps> = ({
  analysisRunInfo,
  replicaSetInfo,
}) => {
  ...
  return (
    <Popover
      headerContent={
        <ResourceLink
          groupVersionKind={{ version: 'v1alpha1', group: 'argoproj.io', kind: 'AnalysisRun' }}
          name={analysisRunInfo.name}
          namespace={replicaSetInfo.namespace}
        />
      }
      bodyContent={<Metrics arInfo={analysisRunInfo} />}
      minWidth={'25.25rem'}
    >
      <Label variant="outline" href="javascript:void(0);" icon={icon} color={color}>
        {analysisRunInfo.shortName}
      </Label>
    </Popover>
  );
};
    ...
    <>
      ...
      >
        <DescriptionListGroup>
          ...
          <DescriptionListDescription>
            <Timestamp simple timestamp={arInfo.startedAt} />
          </DescriptionListDescription>
        </DescriptionListGroup>
        <DescriptionListGroup>
            ...
            <DescriptionListGroup key={index}>
              <DescriptionListTerm>
                <span style={{ display: 'inline-flex', alignItems: 'center' }}>
                  {mr.name}
                  <Tooltip content={getMetricTooltip(arInfo, mr)}>
              ...
              <DescriptionListDescription>
                <Flex>
                  {mr.measurements?.map(function (m, mrIndex) {
                    return (
                      <FlexItem key={mrIndex}>
...
function getMetricTooltip(arInfo: AnalysisRunInfo, mr: MetricResult) {
  ...
  return (
    <table style={{ borderSpacing: '8px 4px', borderCollapse: 'separate' }}>
      ...
      <tr>
        <th>Provider: </th>
        <td>{Object.keys(metric.provider)[0]}</td>
      </tr>
      <tr>
        <th>Status: </th>
        <td>{mr.phase}</td>
      ...
      {metric && metric.successCondition && (
        <tr>
          <th>SuccessCondition: </th>
          <td>{metric.successCondition}</td>
        </tr>
      )}
      {metric && metric.count && (
        <tr>
...
const Measurement: React.FC<MeasurementProps> = ({ measurement }) => {
  ...
  return <Tooltip content={getMeasurementTooltip(measurement)}>{icon}</Tooltip>;
};
  ...
  return (
    <table style={{ borderSpacing: '8px 4px', borderCollapse: 'separate' }}>
      ...
      {measurement.metadata && measurement.metadata['job-name'] && (
        <tr>
          <th style={{ whiteSpace: 'nowrap' }}>Job Name: </th>
          <td>{measurement.metadata['job-name']}</td>
        </tr>
      )}
      <tr>
