...
function getAnalysisRunInfo(
  analysisRuns: AnalysisRunKind[],
  podTemplateHash: string,
  revision: string,
): AnalysisRunInfo[] {
  ...
  analysisRuns.forEach((ar) => {
    if (
      ar.metadata.labels[labelPodTemplateHashKey] == podTemplateHash &&
      ar.metadata.annotations[annotationRevisionKey] == revision
    ) {
...
export const getReplicaSetInfo = async (
  ...
  replicaSets.forEach((rs) => {
    ...
    const podsOwnedByRS: PodKind[] = [];
    let rsStatus = 'Healthy';
    if (rs.spec.replicas && rs.status.availableReplicas < rs.spec.replicas) {
      rsStatus = 'Progressing';
    } else if (rs.spec.replicas === 0 && rs.status.replicas === 0) {
      rsStatus = 'ScaledDown';
    } else {
      rsStatus = 'Healthy';
    }
    // Add pods that each replicaset owns
    pods.forEach((pod) => {
      if (pod.metadata.ownerReferences.find((owner) => owner.name === rs.metadata.name)) {
        if (pod.status.phase !== 'Unknown') {
          podsOwnedByRS.push(pod);
        }
      }
    });
