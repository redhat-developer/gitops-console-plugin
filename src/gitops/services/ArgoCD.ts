import { annotationRefreshKey } from 'src/gitops/utils/gitops';
...
export const syncResourcek8s = async (
  app: ApplicationKind,
  resources: ApplicationResourceStatus[],
): Promise<ApplicationKind> => {
  const syncResources: Resource[] = [];
...
export const syncAppK8s = async (
  ...
  if (app.spec.syncPolicy) {
    ...
    if (app.spec.syncPolicy.syncOptions)
      operation.sync.syncOptions = app.spec.syncPolicy.syncOptions;
    if (app.spec.syncPolicy.automated.prune)
      operation.sync.prune = app.spec.syncPolicy.automated.prune;
  }
  if (resources) {
 ...
 */
export const refreshAppk8s = async (
  app: ApplicationKind,
  hard: boolean,
): Promise<ApplicationKind> => {
