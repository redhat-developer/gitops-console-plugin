...
export const makeReduxID = (k8sKind, query, cluster) => {
    let qs = '';
    if (!_.isEmpty(query)) {
        qs = `---${JSON.stringify(query)}`;
    }
    return `${cluster !== null && cluster !== void 0 ? cluster : 'local-cluster'}${getReferenceForModel(k8sKind || {})}${qs}`;
};
...
export const getIDAndDispatch = (resource, k8sModel, cluster) => {
    var _a;
    ...
    const targetCluster = (_a = resource.cluster) !== null && _a !== void 0 ? _a : cluster;
    const id = makeReduxID(k8sModel, query, targetCluster);
    const dispatch = resource.isList
        ? k8sActions.watchK8sList(id, Object.assign(Object.assign({}, query), { cluster: targetCluster }), k8sModel, null, resource.partialMetadata)
        : k8sActions.watchK8sObject(id, resource.name, resource.namespace, Object.assign(Object.assign({}, query), { cluster: targetCluster }), k8sModel, resource.partialMetadata);
    return { id, dispatch };
};
//# sourceMappingURL=k8s-watcher.js.map